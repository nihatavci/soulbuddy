...
export const AnimatedScrollProgress = memo<IScrollProgress>(
  ({
    children,
    ...
    const resolvedFabWidth = fabWidth ?? screenWidth * 0.7;

    const contentHeight = useSharedValue(1);
    const scrollViewHeight = useSharedValue(1);
    const textTransition = useSharedValue(0);
    ...
    const scrollProgress = useDerivedValue(() => {
      ...
      if (progress >= endReachedThreshold) {
        textTransition.value = withTiming(1, { duration: 300 });
        ...
        textTransition.value = withTiming(0, { duration: 300 });
        if (hasReachedEnd.value) {
      ...
      if (onScrollProgressChange) {
        scheduleOnRN(onScrollProgressChange, clampedProgress);
      }
    ...
    const rAnimatedContainerStyle = useAnimatedStyle<Pick<ViewStyle, 'backgroundColor' | 'transform'>>(() => {
      ...
      return {
        backgroundColor,
        transform: [
          { scale: withSpring(scale, { damping: 12, stiffness: 90, mass: 0.8 }) },
        ],
      };
    });
    ...
    return (
      <>
        ...
        <Animated.View
          style={[
            styles.fab,
            { width: resolvedFabWidth, height: fabHeight, bottom: fabBottomOffset },
            fabAnimatedStyle,
            fabStyle,
          ]}
        >
          ...
          >
            <View style={[DEFAULT_CONTENT_CONTAINER_STYLE, contentContainerStyle]}>
              <Animated.View style={[initialTextStyle, initialContentContainerStyle]}>
                {renderInitialContent()}
              </Animated.View>
